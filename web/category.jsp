...
<!--        <div>test: ${sessionScope.currentUser.id}</div>-->
    ...
    <!--       <script>
        document.addEventListener('DOMContentLoaded', function() {
            var overlayText = localStorage.getItem('overlayText');
            if (overlayText) {
                document.querySelector('.title-container h2').innerText = overlayText;
            }
        });
    
        // Set the overlay text to local storage on link click
        document.querySelectorAll('.category-link').forEach(function(link) {
            link.addEventListener('click', function(event) {
                var overlayText = this.getAttribute('data-overlay');
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (overlayText) {
                document.querySelector('.title-container h2').innerText = overlayText;
                // Optionally clear the localStorage item if it's no longer needed
                // localStorage.removeItem('overlayText');
            }
        });
    </script>
</body>
